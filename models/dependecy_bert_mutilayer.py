...
class BertSelfAttention(nn.Module):
    def __init__(self, config, asp_config):
        ...
        # edge information
        self.e_k = EdgelizedLinear(asp_config.dependency_edge_dim , self.all_head_size)
        self.e_v = EdgelizedLinear(asp_config.dependency_edge_dim , self.all_head_size)
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            if edge_mask is not None:
                ...
                # print("duan dian 2")
                attention_scores = attention_scores.squeeze(-2) + edge_mask
                # print(edge_mask.shape)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        edge_mask: torch.Tensor = self.get_extended_attention_mask(graph_undir, input_shape, device)
        ...
        # exit()
        embedding_edge = self.edge_position_embeddings(edge_matrix_undir)
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, **kwargs):
        ...
        last_linear_dim = (self.asp_config.num_hidden_layers-self.asp_config.DB_begin_layer)*config.hidden_size
        self.classifier = nn.Linear(last_linear_dim, config.num_labels)
        # print(last_linear_dim)
    ...
    def forward(
        ...
        seq_last_output = outputs[0]
        # pooled_output = outputs[1]
        pooled_output = outputs[1]
        all_hidden_out = outputs[2]
        last_output = None
        # print(self.asp_config.DB_begin_layer,self.asp_config.num_hidden_layers)
        for i in range(self.asp_config.DB_begin_layer,self.asp_config.num_hidden_layers):
            aspect_output = self.mask_operation(all_hidden_out[i], aspect_double_idx)
            aspect_output =self.max_pool(aspect_output).squeeze(1)
            # print(aspect_output.shape)
            if last_output ==None:
                last_output = aspect_output
            else:
                last_output = torch.cat([last_output,aspect_output],dim=-1)


        # aspect_output = self.mask_operation(seq_last_output, aspect_double_idx)
        # aspect_output =self.max_pool(aspect_output).squeeze(1)
        # print(aspect_output.shape)
        aspect_output = self.dropout(last_output)
