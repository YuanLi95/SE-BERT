...
class Bilinear(nn.Module):
    ...
    def forward(self, input_1, input2):
        ...
        # print(x.shape,input2.shape)
        output = torch.mul(input_1, input2) # (bs, time_step, dim) * (bs, 1, dim)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, asp_config):
        ...
        self.key = nn.Linear(config.hidden_size, self.all_head_size)
        self.value = nn.Linear(config.hidden_size, self.all_head_size)
        self.k_bilinear = Bilinear(self.all_head_size, self.all_head_size)
        self.v_bilinear = Bilinear(self.all_head_size, self.all_head_size)
    ...
    def forward(
        ...
        if edge_mask is not None :
            ...
            mixed_key_layer = self.k_bilinear(mixed_key_layer, self.K_alpha*edg_k)
            ...
            mixed_value_layer = self.v_bilinear(mixed_value_layer,self.V_alpha*edg_v)
        else:
            # print("duan dian 1")
            mixed_query_layer = mixed_query_layer.unsqueeze(2)
            mixed_key_layer = mixed_key_layer.unsqueeze(1)
