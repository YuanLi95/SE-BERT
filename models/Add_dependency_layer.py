...
from De_Bert import  DeBertlayer
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config,asp_config):
        ...
        self.dependency_embeddings = nn.Embedding.from_pretrained(
            torch.tensor(asp_config.position_matrix, dtype=torch.float),
...
class Add_dependency_Classification(BertPreTrainedModel):
    def __init__(self, config,asp_config, add_pooling_layer=True):
        super().__init__(config,asp_config)
        self.num_labels = config.num_labels
        self.config = config
        self.edge_embedding = Edge_Embeddings(asp_config)
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.Dependency_decoder = nn.ModuleList([DT_layer(config, self.asp_config) for _ in range(self.asp_config.DT_layer_number)])
        self.init_weights()
